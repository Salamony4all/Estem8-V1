...
For each table, identify the column names and all rows of data. It is absolutely crucial that you extract every single row, from the first item to the final Total. Do not omit any rows. Include all summary rows at the end of the table, such as Subtotal, Total, and Discounts.

If the table contains numeric values in an "Amount" column but does not explicitly state the Total and VAT, you must calculate them.
...
Then, you must add these two rows ('Total', 'VAT 5%') to the end of the table data. For these summary rows, place the label (e.g., "Total") in the appropriate column and the calculated value in the amount column, leaving other cells in that row empty.
...
const calculateSummary = (table: Table): Table => {
    ...
    const totalRowFound = table.rows.find(row => row.some(cell => cell.toLowerCase() === 'total'));
    if (totalRowFound) {
      const amountStr = totalRowFound[amountIndex]?.replace(/[^0-9.-]+/g, '');
